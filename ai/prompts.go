...
Generate a title message based on the provided diffs: %s. Review the list of changes and diffs that will be sent to GitHub.
...
Reply only with the one-line title, without any additional text, explanations, or line breaks.`
    GenerateBodyPrompt  = `Generate a body for the branch: %s with the following diffs: %s`
)
