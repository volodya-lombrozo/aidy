...
func TestYAMLConfig_GetGithubAPIKey(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "configtest")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	configFilePath := tempDir + "/config.yml"
	configContent := []byte("github-api-key: gh-api-key")
	err = os.WriteFile(configFilePath, configContent, 0644)
	if err != nil {
		t.Fatalf("Failed to write config file: %v", err)
	}
	...
	apiKey, err := yamlConfig.GetGithubAPIKey()
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	if apiKey != "gh-api-key" {
		t.Fatalf("Expected API key 'gh-api-key', got '%s'", apiKey)
	}
}
